...
from agents.voxactb.perceiver_lang_io import PerceiverVoxelLangEncoder, PerceiverVoxelLang2RobotsEncoder
from agents.voxactb.qattention_peract_bc_agent import QAttentionPerActBCAgent, QAttentionPerActBCAgent2Robots
from agents.voxactb.qattention_stack_agent import QAttentionStackAgent, QAttentionStackAgent2Robots
...
def _get_action(
    ...
    # deal with ignore_collisions 
    # In BimanualObservation，We store it in the right and left arm's UnimanualObservationData
    if obs_tm1.is_bimanual:
        if which_arm == 'right' or dominant_assistive_arm == 'right':
            ignore_collisions = int(obs_tm1.right.ignore_collisions) if obs_tm1.right.ignore_collisions is not None else 0
        elif which_arm == 'left' or dominant_assistive_arm == 'left':
            ignore_collisions = int(obs_tm1.left.ignore_collisions) if obs_tm1.left.ignore_collisions is not None else 0
        else:
            ignore_collisions = 0
    else:
        ignore_collisions = int(obs_tm1.ignore_collisions) if hasattr(obs_tm1, 'ignore_collisions') and obs_tm1.ignore_collisions is not None else 0
    
    if which_arm in SINGLE_ARM or which_arm == 'multiarm' or which_arm == 'dominant' or which_arm == 'assistive':
        if obs_tp1.is_bimanual:
            if which_arm == 'right' or dominant_assistive_arm == 'right':
                gripper_pose = obs_tp1.right.gripper_pose
                gripper_open = obs_tp1.right.gripper_open
            elif which_arm == 'left' or dominant_assistive_arm == 'left':
            ...
            gripper_open = obs_tp1.gripper_open
        ...
        ########## right arm ##########
        gripper_right_pose = obs_tp1.right.gripper_pose
        gripper_right_open = obs_tp1.right.gripper_open
        ...
        ########## left arm ##########
        gripper_left_pose = obs_tp1.left.gripper_pose
        gripper_left_open = obs_tp1.left.gripper_open
...
#             if obs_tp1.target_object_pos is None:
#                 with open("/tmp/debug_fix.log", "a") as f:
#                     f.write(f"Setting missing target_object_pos in launch_utils.py\n")
#                 print("WARNING: target_object_pos is None, using default value")
#                 # obs_tp1.target_object_pos = np.array([0.0, 0.0, 0.5])
...
def _add_keypoints_to_replay(
    ...
    for k, keypoint in enumerate(episode_keypoints):
        ...
        if cfg.method.crop_target_obj_voxel:
            ...
            # set scene bounds based on crop_radius
            if crop_radius == 'auto' and obs_tp1.auto_crop_radius != 0.0:
                rlbench_scene_bounds_processed = utils.get_new_scene_bounds_based_on_crop(obs_tp1.auto_crop_radius, obs_tp1.target_object_pos)
            else:
                # Add cropping for voxact-b
                #debug: see what the scene is cropping besed on
                # print(obs_tp1.target_object_pos)

                # Use the center of the table as the cropping center 
                obs_tp1.target_object_pos = [ 0.22495, -0.0075269, 0.85]
                rlbench_scene_bounds_processed = utils.get_new_scene_bounds_based_on_crop(crop_radius, obs_tp1.target_object_pos)
                # Add another debug
                # print(obs_tp1.target_object_pos)
        ...
        # gripper_pose is different between peract and peract2
        if obs_tp1.is_bimanual:
            if cfg.method.which_arm == 'right' or dominant_assistive_arm == 'right':
                gripper_pose = obs_tp1.right.gripper_pose
            elif cfg.method.which_arm == 'left' or dominant_assistive_arm == 'left':
                ...
                gripper_pose_right = obs_tp1.right.gripper_pose
                gripper_pose_left = obs_tp1.left.gripper_pose
        else:
            # single arm
            gripper_pose = obs_tp1.gripper_pose
        ...
        timeout = False
        if cfg.method.which_arm == 'both':
            # NOTE: action is actually not used
            replay.add(action_right, reward, terminal, timeout, **others)
        else:
            replay.add(action, reward, terminal, timeout, **others)
        obs = obs_tp1
...
def fill_replay(cfg: DictConfig,
                # obs_config: ObservationConfig2Robot,
    ...
    for d_idx in range(num_demos):
        ...
        if cfg.method.keypoint_discovery_no_duplicate:
            ...
            #Change the following to work on the peract2
            ...
            if cfg.method.which_arm in ['both', 'multiarm', 'dominant', 'assistive']:
                # load left and right arm keypoints into 'episode_keypoints'
                episode_keypoints, labels = demo_loading_utils.keypoint_discovery_voxactb(demo, which_arm=cfg.method.which_arm, method=keypoint_method, saved_every_last_inserted=cfg.method.saved_every_last_inserted, dominant_assistive_arm=dominant_assistive_arm, use_default_stopped_buffer_timesteps=cfg.method.use_default_stopped_buffer_timesteps, stopped_buffer_timesteps_overwrite=cfg.method.stopped_buffer_timesteps_overwrite)
            else:
                episode_keypoints = demo_loading_utils.keypoint_discovery_voxactb(demo, which_arm=cfg.method.which_arm, method=keypoint_method, saved_every_last_inserted=cfg.method.saved_every_last_inserted)
