...
PERACTVOX_ROOT = Path(os.environ.get("EXTENSION_ROOT", "/home/hanwen/voxactb_extension/peract2"))
...
class VLM():
    ...
    def __init__(self):
        ...
        if os.path.abspath(os.getcwd()).split('/')[-1] == 'tools':
            ...
            checkpoint = Path(os.environ["PERACTVOX_ROOT"]) / "data" / "segment_anything_vit_h.pth"
            self.debug_output_file_path = "../../../outputs/"
        else:
            checkpoint = Path(os.environ["PERACTVOX_ROOT"]) / "data" / "segment_anything_vit_h.pth"
