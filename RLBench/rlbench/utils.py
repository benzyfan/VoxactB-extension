...
def get_stored_demos(amount: int, image_paths: bool, dataset_root: str,
    ...
    print(f"DEBUG: Checking Demo {i}")
    for j, obs in enumerate(demos):
        if not hasattr(obs, 'target_object_pos') or obs.target_object_pos is None:
            print(f"WARNING: Demo {i}, Observation {j} missing target_object_pos")
